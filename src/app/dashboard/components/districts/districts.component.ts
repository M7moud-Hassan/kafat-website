...
export class DistrictsComponent implements OnInit ,AfterViewInit {
  ...
  getPage(){
    this.service.districtService.getPage(this.pagedRequest).subscribe({
  ...
  addItem(): void {
    const dialogRef = this.service.dialog.open(AddDistrictComponent, {
  ...
  editItem(id:any){
    const dialogRef = this.service.dialog.open(EditDistrictComponent, {
      width:this.windowWidth<767?'99%':(this.windowWidth<1300?'50%':'40%'),
      data:{id:id}
    });
    dialogRef.afterClosed().subscribe(result => {
  ...
  deleteItem(id:number){
    ...
    const dialogRef = this.service.dialog.open(DialogDeleteComponent, {
      ...
      data:{
        ...
        label:'اسم المدينة',
        submit:()=>{
          this.service.districtService.delete(element.id).pipe(
            catchError((error) => {
              console.error(error);
